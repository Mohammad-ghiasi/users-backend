...
export const generateBlog = async (req, res) => {
    try {
        ...
        const response = await axios.post(process.env.MY_AI_ORIGIN, {
            model: "mistral-tiny",
...
export const getBlog = async (req, res) => {
    try {
        ...
        const blog = await Blog.findById(id).populate("user", "firstname lastname email");
        if (!blog) {
...
export const getBlogs = async (req, res) => {
    try {
        const blogs = await Blog.find().populate("user", "firstname lastname email"); // لیست بلاگ‌ها + اطلاعات نویسنده
